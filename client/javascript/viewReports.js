...
exportCSVButton.addEventListener('click', exportAllDataToCSV);
    ...
    submitButtonFines.addEventListener("click", () => {
        ...
        fetch(`/api/reports/getFinesReport/${document.getElementById("year").value}/${userCode}`)
        .then((res) => {
            return res.json();
        })
        .then((data) => {
            ...
            const ctxUserFines = document.getElementById('chartFinesForUser').getContext('2d');
 ...
 const fetchReports = async () => {
   ...
   await fetch(`/api/reports/getIssueVisitorFinesReport/${document.getElementById('year').value}`)
    ...
    const updatedVisitors = data.visitors.map(item => {
       ...
       //Show canvas elements
        ...
        const ctxallFines = document.getElementById('chartAllFines').getContext('2d');
