...
describe('getNotifications', () => {
  it('should fetch notifications successfully', async () => {
    ...
    const userJohn = { name: 'John', surname: 'Doe' };
    const userJane = { name: 'Jane', surname: 'Smith' };
  
    Notification.find.mockResolvedValue(notifications);
    jwt.verify.mockReturnValue({ userCode: '123' });
    User.findOne.mockImplementation((query) => {
      if (query.userCode === 'John Doe') {
        return userJohn;
      } else if (query.userCode === 'Jane Smith') {
        return userJane;
      }
    });
    ...
    expect(User.findOne).toHaveBeenCalledWith({ userCode: 'John Doe' });
    expect(User.findOne).toHaveBeenCalledWith({ userCode: 'Jane Smith' });
