...
exports.checkInVisitor = async (req, res) => {
  try {
    ...
    if (id.length !== 13) {
      return res
        .status(404)
        .json({ error: "ID number must be 13 digits long." });
    }
    ...
    const visitorAlreadyCheckedIn = await Visitor.findOne({
...
exports.getAllVisitors = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Successfully got visitors", visitors: visitors });
  } catch (error) {
...
exports.manageVisitors = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Visitor checked out successfully", visitor: visitor });
  } catch (error) {
